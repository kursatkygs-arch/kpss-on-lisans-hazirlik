...
- scenes: tam olarak {SCENE_COUNT} obje içeren bir liste (yukarıdaki 5 hikaye
  ...
  - scene_prompt_en: bu sahneyi tarif eden kısa, HAREKETLİ bir İngilizce
    görsel açıklama — karakter net bir eylem içinde olsun (örn. "Mimo
    reaching up excitedly toward a red apple on a tree branch"). Asla
    durağan/duruyor bir poz tarif etme. Karakter görünümünü tekrar tarif
    etme, sadece o anki eylemi ve ortamı anlat.
...
def generate_images(episode: dict, output: Path) -> list[Path]:
    ...
    for index, scene in enumerate(episode["scenes"], start=1):
        ...
        _fetch_pollinations_image(prompt, width, height, seed, target)
